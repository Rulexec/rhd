...
- Plugin management system for registering and tracking WebSocket connections as plugins
- Custom event broadcasting and acknowledgment system
...
- Plugins are WebSocket connections that register themselves with a unique `plugin_id`
- Each plugin has an `is_active` flag indicating if the WebSocket connection is still alive
- When a WebSocket disconnects, all plugins registered on that connection are marked as inactive
- Plugins can be explicitly removed via `removePlugin` method
- Plugin list changes are broadcast to subscribers via `pluginRegistered`, `pluginUpdated`, `pluginRemoved` events

### 8. Custom Event System
- Custom events are broadcast to all connected WebSocket clients
- Each custom event has a unique `event_id` (UUID) generated by the server
- Events include `event_name`, `sender_plugin_id`, `additional` (JSON string), and `created_at`
- Plugins can acknowledge events using `ackCustomEvent`
- When an event is acknowledged, a `customEventAcknowledged` event is sent to the original sender
- The server tracks pending acknowledgments per plugin
- `getPendingAcks` returns all events not yet acknowledged by the calling plugin
...
- 20 request methods (createChat, listChats, getChat, deleteChat, updateChat, addMessage, updateMessage, deleteMessage, subscribeChat, unsubscribeChat, subscribeChatsList, unsubscribeChatsList, registerPlugin, getPlugins, subscribePluginsList, unsubscribePluginsList, removePlugin, sendCustomEvent, ackCustomEvent, getPendingAcks)
- 11 event types (chatCreated, chatUpdated, chatDeleted, messageAdded, messageUpdated, messageDeleted, pluginRegistered, pluginRemoved, pluginUpdated, customEvent, customEventAcknowledged)
...
**Key decisions:**
- Plugin registry is shared across all connections via `Arc<RwLock<...>>`
- Each connection tracks which plugins it has registered
- When a connection disconnects, all its plugins are marked as inactive
- Custom events are stored in database with unique event IDs
- Acknowledgments are tracked per plugin per event
- `getPendingAcks` queries events not yet acknowledged by the calling plugin
...
- Unit tests for plugin operations in `rhd_db`
- Unit tests for `rhd_chat_api` serialization/deserialization
- Integration tests for WebSocket protocol
- Tests for subscription broadcasting
- Tests for plugin registration and event broadcasting
- Tests for custom event acknowledgment flow

## Success Criteria

1. Binary package `rhd_chat_server` compiles and runs
2. WebSocket server accepts connections on configurable port
3. All 20 request methods work correctly (using types from `rhd_chat_api`)
4. All 11 event types are emitted to subscribed clients
5. Tags can be assigned to chats and messages
6. Clients can subscribe to specific chats or chats list
7. Database schema extended with tag tables, plugins, and custom events tables
8. Plugins can be registered, listed, and removed
9. Custom events can be sent, broadcast, and acknowledged
10. No AI calls or MCP tool execution in the codebase
11. All tests pass
...
graph TD
    ...
    D --> E[Phase 5: Plugin Management System]
    E --> F[Phase 6: Integration and Testing]

...
use rhd_chat_api::{
    AckCustomEventParams, AddMessageParams, CreateChatParams, GetChatParams, GetPendingAcksParams,
    GetPluginsParams, ListChatsParams, RegisterPluginParams, SendCustomEventParams,
    SubscribeChatParams, UpdateMessageParams, UpdateToolCallTagsParams,
};
...
async fn test_custom_event_context_fields_in_recovery() {
    ...
    let pending = receiver.get_pending_acks(GetPendingAcksParams {}).await.unwrap();
...
async fn test_custom_event_context_fields_with_rejection() {
    ...
    let _cancel_token_3 = client3.on_custom_event(move |_event| {
        let event_received = event_received_3_clone.clone();
        async move {
            event_received.notify_one();
        }

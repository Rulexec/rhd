//! `updateQueueMessage` method types.
//!
//! Partially update a queue message. All fields are optional — only provided fields are changed.
//! Tags use additive/subtractive arrays to avoid races with other clients.

use serde::{Deserialize, Serialize};

/// Parameters for the `updateQueueMessage` method.
///
/// # Example JSON
/// ```json
/// {
...
pub struct UpdateQueueMessageParams {
    /// ID of the queue message to update.
    pub message_id: i64,
...
/// Result of the `updateQueueMessage` method.
///
/// # Example JSON
/// ```json
/// {}
...
mod tests {
    ...
    #[test]
    fn test_update_queue_message_params_serialization() {
        let params = UpdateQueueMessageParams {
            message_id: 456,
        ...
        let deserialized: UpdateQueueMessageParams = serde_json::from_str(&json).unwrap();
        assert_eq!(params, deserialized);
    }

    #[test]
    fn test_update_queue_message_params_optional_fields() {
        let json = r#"{"messageId":456}"#;
        let params: UpdateQueueMessageParams = serde_json::from_str(json).unwrap();
    ...
    #[test]
    fn test_update_queue_message_result_serialization() {
        let result = UpdateQueueMessageResult {};

        let json = serde_json::to_string(&result).unwrap();
        let deserialized: UpdateQueueMessageResult = serde_json::from_str(&json).unwrap();

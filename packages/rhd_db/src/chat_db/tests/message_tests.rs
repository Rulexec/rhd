...
fn test_add_and_get_messages() {
    ...
    let (msg1, _) = db.add_message(chat_id, "user", "Hello", Some("gpt4"), None, true, false).unwrap();
    let (msg2, _) = db.add_message(chat_id, "assistant", "Hi there", Some("gpt4"), None, true, false).unwrap();
    assert_eq!(msg1, 1);
    assert_eq!(msg2, 2);
...
fn test_update_message() {
    ...
    let db = ChatDb::new(path).unwrap();
    let chat_id = db.create_chat("Chat").unwrap();
    let (msg_id, _) = db.add_message(chat_id, "user", "Original", None, None, true, false).unwrap();

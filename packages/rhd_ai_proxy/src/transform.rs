...
pub fn inject_extra_body(
    ...
    serde_json::to_vec(&Value::Object(parsed)).unwrap_or_else(|_| body.to_vec())
}
...
mod tests {
    ...
    fn injects_extra_body_at_top_level() {
        ...
        let forwarded = inject_extra_body("/chat/completions", body.to_string().as_bytes(), &models);
    ...
    fn leaves_non_matching_model_unchanged() {
        ...
        assert_eq!(
            inject_extra_body("/chat/completions", &original, &models),
            original
        );
    }
    ...
    fn leaves_non_completions_path_unchanged() {
        ...
        let original = body.to_string().into_bytes();
        assert_eq!(inject_extra_body("/models", &original, &models), original);
    }
    ...
    fn leaves_invalid_json_unchanged() {
        ...
        assert_eq!(inject_extra_body("/chat/completions", &original, &models), original);
    }
    ...
    fn no_models_configured_is_noop() {
        ...
        assert_eq!(
            inject_extra_body("/chat/completions", &original, &HashMap::new()),
            original
        );
    }
}
